...
await E(() => window.__act.extendSample());
let d2 = await E(() => ({ selected: window.__st.testSummary().selected,
  concluded: window.__st.testSummary().concluded, satisfied: window.__st.testingSatisfied(),
  ex: window.__st.testSummary().exceptions }));
...
await E(() => window.__act.concludeTest('no_rely'));
...
console.log('\nCase E — a questionnaire answer updates coverage; the other two raise open items');
await reset(); await gen();
...
await E(() => {
  ...
  if (st.testingScope().applicable) A.concludeTest('no_rely');
